    ...
    <flux:modal wire:model="showTransactionModal" class="max-w-3xl">
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div class="md:col-span-2">
                    <flux:textarea wire:model.live.debounce.500ms="description" :label="__('Description')"
                        rows="3" />
                </div>
                <div class="md:col-span-1">
                ...
                <div class="md:col-span-1">
                    <flux:input.group :label="__('Amount')">
                        <flux:input wire:model="amount" mask:dynamic="$money($input)" />
                        <flux:input.group.suffix>đ</flux:input.group.suffix>
                    </flux:input.group>
                ...
                <div class="md:col-span-2 space-y-3">
                    @if ($attachment)
                        <div class="rounded-2xl border border-(--color-background) p-4">
                            <div class="flex flex-wrap items-center justify-between gap-3">
                                ...
                                <flux:button size="sm" variant="ghost" color="rose"
                    ...
                    @elseif ($existingAttachment && ! $removeCurrentAttachment)
                        <div class="rounded-2xl border border-(--color-background) p-4">
                            <div class="flex flex-wrap items-center justify-between gap-3">
                                <div class="space-y-1">
                                    <div class="font-medium text-zinc-900 dark:text-white">
                                        {{ basename($existingAttachment) }}</div>
                                    <div class="text-sm text-zinc-500 dark:text-zinc-400">
                    ...
                    @elseif ($removeCurrentAttachment)
                        <flux:callout color="amber" icon="exclamation-circle"
                            :heading="__('Current attachment will be removed after saving.')">
                            <div class="mt-3">
                    ...
                    @if (! $attachment)
                        <flux:file-upload wire:model.live="attachment" :label="__('Upload file')" accept=".pdf">
                            <flux:file-upload.dropzone :heading="__('Drop a file here or click to browse')"
                                :text="__('PDF up to 10MB')" with-progress inline />
                        </flux:file-upload>
                    @endif
